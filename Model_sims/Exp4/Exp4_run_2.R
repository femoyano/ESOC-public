...
T_slope <- 3
nypr    <- c(360)
ypr     <- c(1000, 1250, 1500, 1750, 2000, 2250, 2750, 3000)
...
start_file <- paste0("spin4_", t_save, "_", parsname, "_years", sim_years,
                            savetext, ".RData")
...
temp15 <- Create_T_Input(years = sim_years, T_mean = 15, D_amp = D_amp,
...
RunFun <- function(m,i,j,k) {
  ...
  startvals <- tail(start[start$mode == m & start$tempC == i &
                            start$ypr == j & start$nypr == k , 2:9], 1)
  ...
  input <- Create_ML_Input(years = sim_years, nypr = k, ypr = j, clay = pars[['clay']],
  ...
  if(i != 15 & m == "fixed") {
    input <- Create_ML_Input(years = sim_years, nypr = k, ypr = j,
...
save(all_out, file = paste0("exp4_", t_save, "_", parsname, "_years", sim_years,
                            savetext, ".RData"))
