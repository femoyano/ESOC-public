...
dplot <- all_out
# dplot <- dplot[dplot$moist > 0.08 & dplot$year < 50, ]

dplot$month <- ceiling(dplot$time/720)
dplot$year <- dplot$month/12
dplot$VWC <- as.factor(dplot$VWC)
dplot$C_tot <- dplot$C_P + dplot$C_D + dplot$C_E + dplot$C_M
dplot$group <- interaction(dplot$tempC, dplot$VWC, dplot$mode)

dplot <- ddply(.data = dplot, .variables = .(group), function(df) {
  df$C_rel <- df$C_tot/df$C_tot[1]
  df$C_dif <- df$C_tot-df$C_tot[1]
  return(df)})

# dplot <- foreach(i = unique(exp2_hama$yPr), .combine = 'rbind') %do% {
#   x <- dplot[dplot$yPr == i,]
#   ref <- x[x$tempC == 15 & x$nypr == 90, ]
# }


ggplot(data = dplot, aes(group = group, colour = VWC)) +
  geom_line(mapping = aes(x = year, y = C_P)) +
  # geom_smooth(mapping = aes(x = year, y = C_rel)) +
  facet_wrap(~tempC) +#(tempC~mode) + #, scales = "free_y") + #
...
ggplot(data = dplot2, aes(group = Temperature, colour = Temperature)) +
  scale_color_manual(values = c('grey0', 'grey30', 'grey50', 'grey65', 'grey80', 'grey90'),
                     name = expression(paste(degree,"C"))) +
  geom_line(mapping = aes(x = VWC, y = C_P_rel), size = 1.2) +
  ...
  xlab(expression(paste("Moisture (",m^3*m^-3,")"))) +
...
ggplot(data = dplot2, aes(group = Temperature, colour = Temperature)) +
  scale_color_manual(values = c('grey0', 'grey30', 'grey50', 'grey65', 'grey80', 'grey90'),
                     name = expression(paste(degree,"C"))) +
  geom_line(mapping = aes(x = VWC, y = C_P_dif), size = 1.2) +
  facet_grid(.~mode, scales = "free_y") + # scales = "free_y"
  theme_bw(base_size = 12) +
  theme(panel.grid.major = element_blank(),
        #panel.grid.minor = element_blank(),
