...
start_file <- paste0("spin3_", t_save, "_", parsname, "_years", sim_years,
                     savetext, ".RData")
...
RunFun <- function(m,i,j) {
  ...
  startvals <- tail(start[start$mode == m & start$tempC == i & start$moist == j, 2:9], 1)
...
save(all_out, file = paste0("exp3_", t_save, "_", parsname, "_years", sim_years,
                            savetext, ".RData"))
