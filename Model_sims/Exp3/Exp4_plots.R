...
ggplot(data = dplot, aes(group = ypr, colour = ypr)) +
  geom_line(mapping = aes(x = year, y = C_tot)) +
...
ggplot(data = dplot2, aes(group = Temperature, colour = Temperature)) +
  ...
  geom_line(mapping = aes(x = ypr, y = C_P_rel), size = 1.2) +
  ...
  ylab("Relative SOC change") +
  geom_hline(yintercept = 1, lty = 2)

ggplot(data = dplot2, aes(group = Temperature, colour = Temperature)) +
  scale_color_manual(values = c('grey0', 'grey30', 'grey50', 'grey65', 'grey80', 'grey90'),
                     name = expression(paste(degree,"C"))) +
  geom_line(mapping = aes(x = ypr, y = C_P_dif), size = 1.2) +
  facet_grid(.~mode, scales = "free_y") + # scales = "free_y"
  theme_bw(base_size = 12) +
  theme(panel.grid.major = element_blank(),
        #panel.grid.minor = element_blank(),
  ...
  ylab(expression(paste("SOC change (kg  ",m^-2, ")"))) +
